    ...
    req.on('end', () => {
      try {
        ...
        if (filePath) {
          ...
          const ALLOWED_DIR = path.resolve('./allowed');
          const requested = path.resolve(ALLOWED_DIR, filePath);
          if (!requested.startsWith(ALLOWED_DIR + path.sep)) {
          ...
          fs.readFile('./config.json', 'utf8', (configErr, configData) => {
            ...
            fs.readFile(requested, 'utf8', (error, stdout) => {
              if (error) {
